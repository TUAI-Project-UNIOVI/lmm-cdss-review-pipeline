...
**Status: grey areas ratified 2026-07-28; screening not yet started.** Records entering Full-Text Screening: 157. Full text not retrievable (access exclusion): 9, pre-filled as `OT5`/`EXCLUDE` in both reviewer workbooks (deterministic, no reviewer judgment required). Records available for screening: 148. Screening checklist, codes, and grey-area (GA) definitions are in `screening/SCREENING_GUIDE.md § Key definitions`.

**Grey-area resolution.** GA1, GA5, GA7, and GA9 — drafted 2026-07-09/10, marked DECIDED 2026-07-13, reopened to OPEN 2026-07-15 — were ratified 2026-07-28 and are now stable in `screening/SCREENING_GUIDE.md § Key definitions`. GA7 (foundational): a clinical decision output is any patient-specific content (real or simulated patient) the LLM itself generates that constitutes, contributes to, or grounds a step in the decision cascade; need not be the full/final/acted-upon decision, need not be prospective. GA1/GA5/GA9 (LLM role sufficiency): the LLM must itself occupy a step in the cascade (as the CDSS, a processed input feeding it, or an orchestrator of tools/agents) rather than sit outside it as builder/designer/selector (GA2) or as an off-line knowledge-maintenance layer (GA5, always EXCLUDE); workflow position (GA9) does not affect CO3 status and is recorded only for later charting.

**Reviewer guidance.** `screening/reviewer_guide_phase1.md` (renamed from `reviewer_guide.md`) covers Phase 1; `screening/reviewer_guide_phase2.md` is a new document covering Phase 2 full-text screening, including a standardized Claude Opus 5 reading-aid prompt (fixed opening instruction, one fixed question per check — PO1, CO2, CO3a/CO3b, CX4 — and three fixed conditional follow-ups for the documentation/evaluation-framework/pipeline-role grey areas) so both reviewers' AI-assisted reading sessions are comparable for PRISMA-trAIce reporting (see AI-Use Log entry 5).
...
| 5 | Stage 4 — Full-Text Screening Preparation | Ratification support for grey areas GA1, GA5, GA7, GA9 (drafting candidate definitions and worked examples for user review/decision) in `screening/SCREENING_GUIDE.md § Key definitions`; drafting of `screening/reviewer_guide_phase2.md` (Phase 2 reviewer instructions, including the standardized Claude Opus 5 reading-aid prompt reviewers will use during screening itself — see below) | Claude Code (Anthropic) | claude-sonnet-5 | Interactive pair-programming via CLI; user directed each definition and made all final ratification decisions (GA5/GA9 confirmed by user directly; GA1/GA7 drafted by Claude, ratified by user); Claude drafted document text, user reviewed, corrected, and approved every change | 2026-07-28 | Human (user made every substantive definitional and inclusion/exclusion decision; Claude's role was drafting text and structure for user review — no GA definition was finalized without explicit user confirmation) |
| 6 | Stage 4 — Full-Text Screening (reviewer reading aid) | Reviewers may consult Claude Opus 5, per the standardized prompt in `screening/reviewer_guide_phase2.md § Standard reading-aid prompt`, as an optional reading aid to answer clarifying questions about a full text before reaching a PO1/CO2/CO3/CX4/OT5 decision. The tool never issues an include/exclude recommendation; the reviewer's decision is independent judgment | Claude (claude.ai or Claude Code, reviewer's choice) | claude-opus-5 (fixed, both reviewers) | Fixed scripted prompt: one standard opening instruction plus fixed per-check questions (PO1, CO2, CO3a, CO3b, CX4) and three fixed conditional follow-ups (documentation/note-generation, evaluation/benchmark task, LLM pipeline role); no free-form prompting | Screening not yet started as of 2026-07-28; to be logged per-record in each reviewer's workbook `rationale` field when used | Human (reviewer decision is independent of the tool's answers; tool is reading aid only) |
